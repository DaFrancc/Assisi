...
// GLSL disallows storing a combined sampler2D in a local variable — the
// texture/sampler combine has to happen inline at each call site. This
// wrapper covers the plain texture() lookups below; textureOffset() calls
// can't be wrapped the same way since its offset argument must be a
// compile-time constant literal at the call site, which doesn't survive
// being passed through a function parameter — those stay inlined.
vec4 Sample(vec2 uv)
{
    return texture(sampler2D(uScreenTexture, uScreenSampler), uv);
}

void main()
{
    vec3  colorCenter = Sample(vTexCoords).rgb;
    float lumaCenter  = Luma(colorCenter);

    // Cardinal neighbours
    float lumaN = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2( 0,  1)).rgb);
    float lumaS = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2( 0, -1)).rgb);
    float lumaE = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2( 1,  0)).rgb);
    float lumaW = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2(-1,  0)).rgb);
    ...
    // Diagonal neighbours
    float lumaNW = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2(-1,  1)).rgb);
    float lumaNE = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2( 1,  1)).rgb);
    float lumaSW = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2(-1, -1)).rgb);
    float lumaSE = Luma(textureOffset(sampler2D(uScreenTexture, uScreenSampler), vTexCoords, ivec2( 1, -1)).rgb);
    ...
    float stepLen = isHoriz ? pc.texelSize.y : pc.texelSize.x;
    ...
    vec2  step  = isHoriz ? vec2(pc.texelSize.x, 0.0) : vec2(0.0, pc.texelSize.y);
    vec2  uv1   = uv - step;
    vec2  uv2   = uv + step;
    float end1  = Luma(Sample(uv1).rgb) - lumaLocalAvg;
    float end2  = Luma(Sample(uv2).rgb) - lumaLocalAvg;
    bool  done1 = abs(end1) >= gradScaled;
    bool  done2 = abs(end2) >= gradScaled;

    for (int i = 0; i < SEARCH_STEPS && !(done1 && done2); ++i)
    {
        if (!done1) { uv1  -= step * QUALITY[i]; end1 = Luma(Sample(uv1).rgb) - lumaLocalAvg; done1 = abs(end1) >= gradScaled; }
        if (!done2) { uv2  += step * QUALITY[i]; end2 = Luma(Sample(uv2).rgb) - lumaLocalAvg; done2 = abs(end2) >= gradScaled; }
    }
    ...
    FragColor = vec4(Sample(finalUV).rgb, 1.0);
}
