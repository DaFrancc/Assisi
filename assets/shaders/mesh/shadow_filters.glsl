...
// ---- Tent filter --------------------------------------------------------------
//
// A tent of half-width w texels, integrated over each texel's own square, gives
// that texel's weight. The weights slide continuously as the lookup moves across
// a texel, so an edge filtered this way is a smooth ramp at any magnification
// rather than a staircase, and there is no per-pixel noise to hide. A half-width
// of 1.5 is the 3x3 tent PCF Unity and Filament ship; 2.5 is their 5x5.

// The share of a unit-area tent of half-width @p w that lies left of @p x.
float TentCdf(float x, float w)
{
    float a = clamp(x / w, -1.0, 1.0);
    return a < 0.0 ? 0.5 * (1.0 + a) * (1.0 + a) : 1.0 - 0.5 * (1.0 - a) * (1.0 - a);
}

// The weight of the texel spanning [@p k, @p k + 1) under a tent of half-width
// @p w centred at @p t, all in texels. Zero for a texel the tent does not reach,
// and the weights of every texel along an axis sum to one.
float TentTexelWeight(float k, float t, float w)
{
    return TentCdf(k + 1.0 - t, w) - TentCdf(k - t, w);
}
