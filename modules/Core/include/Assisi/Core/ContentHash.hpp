...
///
/// For any hash two machines must agree on. Reads in binary so the platform
/// translates nothing behind our back, then normalises explicitly: text reaches
...
/// but semantically identical file, so hashing raw bytes makes a Windows and a
/// Linux peer refuse each other over the same level.
///
...
///
/// **Every content hash compared across machines must come from here.** A second
/// spelling of it is not a style problem: it is a pair of peers that refuse each
/// other over an identical file.
