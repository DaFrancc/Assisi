...
///
/// Two things are different, and both are deliberate. Duplicate *names* are an
/// error rather than a tolerated collision — two message types with the same
/// name share a wire identity and misdispatch across the wire, which is silent
/// corruption of exactly the kind the protocol hash exists to prevent.
...
public:
    ...
    ///
    /// A second registration under an existing name is caught at finalize, not
    /// here: two messages sharing a name share a wire identity, and every packet
    /// naming it would reach the wrong handler on one of the two machines.
