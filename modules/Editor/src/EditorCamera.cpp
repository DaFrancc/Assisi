...
void EditorApp::HandleEntityPicking()
{
    ...
    if (GetActions().IsActionPressed("Select", input) && !input.IsMouseCaptured() && !ImGuiWantsMouse() &&
        !IsUsingGizmo())
    {
        float entityT = 0.f;
...
void EditorApp::ApplyEyedropperPick(Assisi::ECS::Entity picked)
{
    ...
    const void *ptr = _eyedropperMeta->getByEntity(_scene, _eyedropperEntity.index, _eyedropperEntity.generation);
    if (!ptr)
        return;
    ...
    if (history != nullptr)
        history->RecordBefore(_eyedropperEntity, _eyedropperMeta->id, EditLabel("Assign reference", _eyedropperEntity),
                              _eyedropperEntity);

    auto *field = reinterpret_cast<Assisi::ECS::Entity *>(const_cast<char *>(static_cast<const char *>(ptr)) +
                                                          _eyedropperFieldOffset);
    *field = picked;
...
void EditorApp::UpdateCamera(float dt)
{
    ...
    {
        if (GetActions().IsActionPressed("LookMode", input) && !imguiWantsMouse)
        {
    ...
    if (GetActions().IsActionPressed("LookMode", input) && !imguiWantsMouse)
    {
        input.SetInputMode(Assisi::Window::InputMode::Game);
    }
    if (GetActions().IsActionReleased("LookMode", input))
    {
        input.SetInputMode(Assisi::Window::InputMode::GameAndUi);
    }
    ...
    if (input.IsMouseCaptured())
    {
        ...
        const glm::vec3 forward = {glm::cos(glm::radians(_pitch)) * glm::cos(glm::radians(_yaw)),
        ...
        glm::vec3 move{0.f};
        if (GetActions().IsActionDown("MoveForward", input))
        {
            move += forward;
        }
        if (GetActions().IsActionDown("MoveBackward", input))
        {
            move -= forward;
        }
        if (GetActions().IsActionDown("MoveRight", input))
        {
            move += right;
        }
        if (GetActions().IsActionDown("MoveLeft", input))
        {
            move -= right;
        }
        if (GetActions().IsActionDown("MoveUp", input))
        {
            move.y += 1.f;
        }
        if (GetActions().IsActionDown("MoveDown", input))
        {
            move.y -= 1.f;
        }
...
Assisi::ECS::InstanceId EditorApp::PickInstance(glm::vec2 mousePos, float &tOut)
{
    ...
    {
        float t = 0.f;
        if (RayBillboardIntersect(ray.origin, ray.direction, row->transform.position, ray.cameraRight, ray.cameraUp,
                                  iconHalf, t) &&
            t < tOut)
        {
            tOut = t;
