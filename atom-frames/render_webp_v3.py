#!/usr/bin/env python3
"""Render the v2 atom-spinner to an animated, motion-BLURRED WebP.

This is the raster counterpart to build_font_v2.py. Where the font bake must drop
...
WEBP_FRAMES = 120                  # v3: 120 samples over the SAME 2 s LOOP_MS (independent
                                   # of the font's N_FRAMES); ~17ms/frame = 60 fps
D = m.SIZE * SS                    # working canvas size
WEBP_OUT = os.path.join(m.HERE, "atom-spinner-v3.webp")
...
def main():
    m.build_schedule()
    m.measure_speeds()
    rendered = [render_frame(f / WEBP_FRAMES) for f in range(WEBP_FRAMES)]
    ...
    duration = round(m.LOOP_MS / WEBP_FRAMES)         # ms per frame (~21)
    ...
    frames[0].save(WEBP_OUT, format="WEBP", save_all=True, append_images=frames[1:],
                   duration=duration, loop=0, method=6, quality=82, alpha_quality=70)
    print("wrote {}  ({} frames, {}px, {}ms/frame, looped)".format(
        WEBP_OUT, WEBP_FRAMES, OUT_SIZE, duration))
