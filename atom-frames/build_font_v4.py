#!/usr/bin/env python3
"""Build the v4 atom-spinner: SVG frames + Spinner4-NN.ttf, from one set of math.

v4 = v3 on a LONG loop with a single spin at the very end:

  * the loop is 10 s (v3: 2 s);
  * the protons keep their v3 tempo -- the Kepler schedule runs ORBIT_CYCLES (5)
    full cycles across the loop, so the orbital motion is pixel-for-pixel the v3
    motion, just repeated five times;
  * the whole-atom spin fires ONCE, in the last SPIN_MS of the loop, and lands
    back on 0 exactly at the loop boundary.

So: ~8.5 s of quiet orbiting, then one whip-spin, then it repeats seamlessly.

Three frame densities are built from the same math, selected by CLI arg:
    python build_font_v4.py 15 | 30 | 60      (fps -> 150 / 300 / 600 frames)
each writing its own Spinner4-NN.ttf. SVG frames are only written with --svg
(600 SVGs per variant is rarely what you want).
...
# --- timing / spin window ------------------------------------------------------
LOOP_MS = 10000                # v4: one full loop of the whole animation (10 s)
ORBIT_CYCLES = 5               # Kepler cycles per loop. LOOP_MS / ORBIT_CYCLES = 2000 ms
                               # per cycle == v3's LOOP_MS, so the protons move at exactly
                               # the v3 tempo; only the SPIN is stretched out to once/loop.

# Frame-density variants. v4 is 5x longer than v3, so 60 fps means 600 glyphs; the
# lower-fps variants trade smoothness (most visible during the fast end-spin) for size.
VARIANTS = {15: 150, 30: 300, 60: 600}      # fps -> frame count over LOOP_MS
FPS = 30                       # default; overridden by configure()
N_FRAMES = VARIANTS[FPS]
...
SPIN_MS = 1500                 # duration of the single spin -- the same 1500 ms window
                               # v3 used, so the whip reads at the identical speed.
SPIN_END_MS = LOOP_MS          # ...and it ENDS on the loop boundary, i.e. the spin is the
SPIN_START_MS = SPIN_END_MS - SPIN_MS   # last thing that happens each loop (8500 -> 10000).
...
# The spin eases a full 360 and the shrink returns to 1.0 at w=1, so frame 0 of the next
# loop is continuous with the last frame -- and ORBIT_CYCLES is an integer, so the proton
# schedule wraps on the same boundary. The loop is seamless.
# (render_webp_v4 imports all of this, so the WebP and the TTF run the SAME motion math --
#  the only thing the font can't reproduce is the WebP's motion blur, which is a rendering
#  feature, not part of the math.)
...
FAMILY = "Spinner4-30"         # set by configure()

# --- Kepler schedule lookup table: tau (loop fraction) -> eccentric angle t -----
TABLE_N = 2048

HERE = os.path.dirname(os.path.abspath(__file__))
SVG_DIR = os.path.join(HERE, "frames-v4-30")
TTF_OUT = os.path.join(HERE, "Spinner4-30.ttf")


def configure(fps):
    """Select a frame-density variant. Sets the frame count and the output names, and
    clears the geometry cache (it is keyed by frame index, which is variant-relative).
    render_webp_v4 calls this too, so both outputs of a variant stay in lockstep."""
    global FPS, N_FRAMES, FAMILY, SVG_DIR, TTF_OUT
    if fps not in VARIANTS:
        raise SystemExit("fps must be one of {}".format(sorted(VARIANTS)))
    FPS = fps
    N_FRAMES = VARIANTS[fps]
    FAMILY = "Spinner4-{}".format(fps)
    SVG_DIR = os.path.join(HERE, "frames-v4-{}".format(fps))
    TTF_OUT = os.path.join(HERE, "Spinner4-{}.ttf".format(fps))
    _geom_cache.clear()
...
def orbit_state(k, tau):
    ...
    tau is the fraction of the WHOLE (10 s) loop; the Kepler schedule is sampled at
    tau * ORBIT_CYCLES, so the particles run five v3-speed cycles per v4 loop instead
    of being stretched out over it."""
    p = tau * ORBIT_CYCLES + k * PHASE_STEP
    t = eccentric_angle(p)
...
def main():
    argv = sys.argv[1:]
    want_svg = "--svg" in argv
    fps = [int(a) for a in argv if a.isdigit()] or sorted(VARIANTS)   # no arg -> all three
    build_schedule()
    measure_speeds()
    for f in fps:
        configure(f)
